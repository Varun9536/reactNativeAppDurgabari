...
const fetchTransportEventsPeriodically = async (setGroupedEvents, setLoading, setError, currentGrouped) => {
    try {
...
export default function Transport() {
    ...
    const loadData = () => {
        setLoading(true);
        setError(null);
        fetchTransportEventsPeriodically(setGroupedEvents, setLoading, setError, groupedEvents);
    }

    const onRefresh = useCallback(() => {
        setRefreshing(true);
        fetchTransportEventsPeriodically(setGroupedEvents, () => { }, setError, groupedEvents)
        ...
        return (
            <BackgroundGradient>
                <View style={styles.centered}>
                    ...
                    <Text style={styles.loadingText}>Loading Transport events...</Text>
                </View>
            </BackgroundGradient>
        );
    }
    ...
    if (dateKeys.length === 0) {
        return (
            <BackgroundGradient>
                <View style={styles.centered}>
                    <Icon name="bus-outline" size={50} color="#84119dff" />
                    <Text style={styles.emptyText}>No upcoming Transport events.</Text>
    ...
    return (
        <BackgroundGradient>
            <ScrollView
                ...
                refreshControl={
                    <RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={["#84119dff"]} tintColor="#84119dff" />
                }
            >

                <View style={styles.header}>
                    <Text style={styles.appTitle}>Transportation</Text>
                    <Text style={styles.appSubtitle}>DurgaBari Transportation And Parking</Text>
                </View>
                {dateKeys.map(date => (
